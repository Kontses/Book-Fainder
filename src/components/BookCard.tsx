...
export const BookCard = ({ title, author, description, year, coverUrl, isbn, onSave, isSaved = false, variant = "default" }: BookCardProps) => {
  ...
  const handleAppleBooksClick = async () => {
    ...
    try {
      ...
      toast.error('Failed to search Apple Books');
    } finally {
        ...
        <div className="flex-1 flex flex-col">
          ...
          <CardContent className={cn("p-0 flex-1 flex flex-col justify-between", !isCompact && "px-4")}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-auto">
              <Button
                className={cn(
                  "w-full bg-[#FF9900] hover:bg-[#FF9900]/90 text-black font-semibold transition-all duration-300 hover:shadow-lg px-2 sm:px-4 text-xs sm:text-sm",
                  isCompact && "text-xs py-2"
                )}
                onClick={() => window.open(getAmazonLink(), '_blank', 'noopener,noreferrer')}
              >
                <ShoppingCart className={cn("mr-1 sm:mr-2", isCompact ? "h-3 w-3 sm:h-4 sm:w-4" : "h-3 w-3 sm:h-5 sm:w-5")} />
                Amazon
              </Button>
              <Button
                className={cn(
                  "w-full bg-black hover:bg-black/90 text-white font-semibold transition-all duration-300 hover:shadow-lg px-2 sm:px-4 text-xs sm:text-sm",
                  isCompact && "text-xs py-2"
                )}
                onClick={handleAppleBooksClick}
              ...
              >
                {isAppleLoading ? (
                  ...
                  <svg
                    ...
                    className={cn("mr-1 sm:mr-2 fill-current", isCompact ? "h-3 w-3 sm:h-4 sm:w-4" : "h-3 w-3 sm:h-5 sm:w-5")}
                  >
